...
const DOCTORS_MAP: Record<string, { name: string; specialty: string; image: string }> = {
  ...
  '2': { name: 'دکتر علی وارسته', specialty: 'متخصص ارتوپدی', image: '/images/dr2.jpg' },
  '3': { name: 'دکتر بهنوش حسینی', specialty: 'جراح گوش و حلق و بینی', image: '/images/dr3.png' },
  '4': { name: 'دکتر علی راد', specialty: 'متخصص ریه', image: '/images/dr4.png' },
  '5': { name: 'دکتر لعیا زنگنه', specialty: 'متخصص قلب و عروق', image: '/images/dr11.png' },
  '6': { name: 'دکتر یاشار پناهی', specialty: 'متخصص روانشناسی بالینی', image: '/images/dr12.png' },
  '7': { name: 'دکتر زهرا سعادتی', specialty: 'متخصص گوش و حلق و بینی', image: '/images/dr13.png' },
  '8': { name: 'دکتر ماهان گروسی', specialty: 'متخصص دندانپزشکی', image: '/images/dr14.png' },
}

interface AppointmentData {
  _id?: string
  doctorId?: { _id?: string; fullName?: string; specialty?: string; avatar?: string }
  date?: string
  startTime?: string
  status?: string
  trackingCode?: string
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    const national = localStorage.getItem('userNationalCode') || ''
    const birth = localStorage.getItem('userBirthDate') || ''

    if (name) setUserName(name)
    if (phone) setUserPhone(phone)
    setAvatarUrl(localStorage.getItem('userAvatar'))

    if (fullNameRef.current) fullNameRef.current.value = name
    if (nationalCodeRef.current) nationalCodeRef.current.value = national
    if (birthDateRef.current) birthDateRef.current.value = birth
  ...
  const saveProfile = () => {
    ...
    const nationalCode = nationalCodeRef.current?.value.trim() || ''
    ...
    localStorage.setItem('userNationalCode', nationalCode)
  ...
  const userInitial = userName.charAt(0) || 'ک'
  ...
  return (
    <div className="bg-[#F9FAFB]" style={{ fontFamily: "'Vazirmatn', sans-serif", minHeight: '100vh' }}>
      ...
      <div className="max-w-[1216px] mx-auto px-4 sm:px-6 lg:px-[112px]" style={{ paddingTop: '120px', paddingBottom: 'calc(80px + 40px)' }}>
        <h2 className="text-[24px] font-medium text-[#262626] leading-[38px] text-right mb-8">پروفایل کاربری</h2>

        {/* Desktop: side by side, Mobile: stacked */}
        <div className="flex flex-col lg:flex-row gap-8">
          {/* Profile Card */}
          <div className="lg:w-[225px] w-full shrink-0">
            <div className="border border-[#E7E7E7] rounded-[12px] p-4 flex lg:flex-col items-center lg:items-start gap-4 lg:gap-4 bg-white">
              <div className="relative w-[80px] h-[80px] lg:w-[193px] lg:h-[193px] rounded-full bg-[#F0F5FE] flex items-center justify-center shrink-0 overflow-hidden cursor-pointer group" onClick={() => fileInputRef.current?.click()}>
                {avatarUrl ? (
                  <img src={avatarUrl} alt={userName} className="w-full h-full object-cover" />
                ) : (
                  <span className="text-[32px] lg:text-[48px] font-bold text-[#4179F0]">{userInitial}</span>
                )}
              ...
              <div className="flex flex-col items-center lg:items-start gap-2 w-full">
                <span className="text-[16px] lg:text-[18px] font-medium text-[#262626] leading-[28px]">{userName}</span>
                <Link href="/appointments" className="flex justify-center items-center p-[10px] gap-[6px] w-full h-[40px] border border-[#4179F0] rounded-[8px] text-[14px] font-medium text-[#4179F0] leading-[22px] hover:bg-[#F0F5FE] transition no-underline">
                  نوبت‌های من
                </Link>
              </div>
            </div>
          ...
          {/* Info Card */}
          <div className="flex-1 border border-[#E7E7E7] rounded-[12px] p-6 lg:p-8 bg-white">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-x-8 lg:gap-y-10">
              <div className="flex flex-col items-end gap-1">
                <label className="text-[16px] lg:text-[18px] font-medium text-[#4F4F4F] leading-[28px] text-right">نام و نام خانوادگی</label>
                <div className="flex items-center p-3 lg:p-4 w-full h-[48px] bg-white rounded-[8px] border border-[#E7E7E7]">
                  <input type="text" ref={fullNameRef} className="w-full text-[14px] lg:text-[16px] font-medium text-[#262626] text-right bg-transparent outline-none border-none" placeholder="نام خود را وارد کنید" />
                </div>
              </div>
              <div className="flex flex-col items-end gap-1">
                <label className="text-[16px] lg:text-[18px] font-medium text-[#4F4F4F] leading-[28px] text-right">شماره موبایل</label>
                <div className="flex items-center p-3 lg:p-4 w-full h-[48px] bg-white rounded-[8px] border border-[#E7E7E7]">
                  <input type="text" value={userPhone} disabled className="w-full text-[14px] lg:text-[16px] font-medium text-[#262626] text-right bg-transparent outline-none border-none opacity-60" />
                </div>
              </div>
              <div className="flex flex-col items-end gap-1">
                <label className="text-[16px] lg:text-[18px] font-medium text-[#4F4F4F] leading-[28px] text-right">کد ملی</label>
                <div className="flex items-center p-3 lg:p-4 w-full h-[48px] bg-white rounded-[8px] border border-[#E7E7E7]">
                  <input type="text" ref={nationalCodeRef} className="w-full text-[14px] lg:text-[16px] font-medium text-[#262626] text-right bg-transparent outline-none border-none" placeholder="۱۲۳۴۵۶۷۸۹۰" />
                </div>
              </div>
              <div className="flex flex-col items-end gap-1">
                <label className="text-[16px] lg:text-[18px] font-medium text-[#4F4F4F] leading-[28px] text-right">تاریخ تولد</label>
                <div className="flex items-center p-3 lg:p-4 w-full h-[48px] bg-white rounded-[8px] border border-[#E7E7E7]">
                  <input type="text" ref={birthDateRef} className="w-full text-[14px] lg:text-[16px] font-medium text-[#262626] text-right bg-transparent outline-none border-none" placeholder="۱۳۷۰/۰۱/۰۱" />
                </div>
              </div>
            </div>
            <div className="flex justify-end mt-8">
              <button onClick={saveProfile} disabled={saving} className="flex justify-center items-center p-[12px] gap-[8px] w-[144px] h-[48px] bg-[#4179F0] rounded-[8px] text-[16px] font-medium text-white leading-[25px] hover:bg-[#3568D6] transition disabled:opacity-50">
